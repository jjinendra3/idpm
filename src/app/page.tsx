  ...
  return (
    <div className="max-w-xl mx-auto mt-8 bg-white rounded-2xl shadow-lg overflow-hidden border" style={{ height: "600px", display: "flex", flexDirection: "column" }}>
      <header className="px-4 py-3 border-b">
        ...
        <div className="text-sm text-gray-500">Send messages locally — hook up a backend later</div>
      </header>

      <div ref={listRef} className="flex-1 p-4 overflow-y-auto" style={{ background: "#f7fafc" }}>
        {messages.map((m) => (
          <div key={m.id} className={`mb-3 flex ${m.sender === "you" ? "justify-end" : "justify-start"}`}>
            <div style={{ maxWidth: "78%" }}>
              <div className={`inline-block px-4 py-2 rounded-lg ${m.sender === "you" ? "bg-blue-600 text-white" : "bg-white text-gray-900 border"}`}>
                <div style={{ whiteSpace: "pre-wrap" }}>{m.text}</div>
              </div>
              <div className={`text-xs mt-1 ${m.sender === "you" ? "text-right text-gray-300" : "text-left text-gray-500"}`}>{m.time}</div>
            </div>
          </div>
        ))}
      </div>
